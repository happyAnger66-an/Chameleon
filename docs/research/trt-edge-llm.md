...
flowchart TB
    subgraph init ["初始化"]
        ...
        B3[共享 execution context memory]
    end
...
4. LLM / vision / action **串行执行**，共用一块 `max(base, vision, action)` 的 **execution context memory**（`setContextMemory`），降低端侧显存峰值。
...
┌─────────────────────────────────────────────────────────────┐
│ 系统级                                                       │
│  · 三 engine 串行 + 共享 context memory（省显存）              │
