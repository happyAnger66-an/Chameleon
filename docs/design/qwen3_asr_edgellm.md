...
> 状态：Phase 1–3 已落地（架构/deploy/runtime/ASR 评测/流式 V1/可选 quantize）。
...
1. **优先 wav/mp3/flac 直喂**——现行 C++ runtime 内置 mel 提取（见 Edge-LLM `asr.md`）；历史「mel 必须 Python 落盘 safetensors」表述已过时。Chameleon 默认传音频路径；Python Whisper FE 仅作流式/调试回退；
...
- **音频输入流式**：**不支持**——runtime 只接受完整 utterance（wav 或 mel），无 chunked audio prefill、无跨请求增量 encoder。
...
- **音频输入**：默认传 wav/mp3/flac 路径，由 Edge-LLM C++ runtime 做 mel；Python Whisper FE 仅作可选回退；
...
| mel 落盘 / 高频 IO（旧约束） | **现行路径 wav 直喂**，不再强制 Python mel；调试仍可落盘 |
