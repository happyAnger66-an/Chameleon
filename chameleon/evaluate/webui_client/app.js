...
/** 各 dim 已推送 step 的 diff MAE/MSE 累计均值（Chameleon evaluate）。 */
let latestMaeDimMean = null; // number[] | null
let latestMseDimMean = null; // number[] | null
...
function setCompareLayoutVisible(on) {
  ...
  if (!on) refreshPtDimDiffMeanTable();
  updateMetricChartWrapTitles();
  setDimTraceToggleUi();
}

function fmtDiffMeanCell(v) {
  if (typeof v !== "number" || !Number.isFinite(v)) return "—";
  return v.toFixed(6);
}

/** 按维累计 PT↔GT：mae_dim_mean / mse_dim_mean（流式均值） */
function refreshPtDimDiffMeanTable() {
  const tbody = el("ptDimDiffMeanBody");
  const card = el("ptDimDiffMeanCard");
  if (!tbody || !card || dualPredMode()) return;
  const maeArr = latestMaeDimMean;
  const mseArr = latestMseDimMean;
  tbody.innerHTML = "";
  for (const d of [...state.dims].sort((a, b) => a - b)) {
    ...
    tdMae.textContent =
      Array.isArray(maeArr) && typeof maeArr[d] === "number" ? fmtDiffMeanCell(maeArr[d]) : "—";
    tr.appendChild(tdMae);
    const tdMse = document.createElement("td");
    tdMse.textContent =
      Array.isArray(mseArr) && typeof mseArr[d] === "number" ? fmtDiffMeanCell(mseArr[d]) : "—";
    tr.appendChild(tdMse);
    tbody.appendChild(tr);
  }
}

function updatePerDimDiffMeanFromStep(event) {
  const met = event.metrics;
  if (!met || typeof met !== "object" || dualPredMode()) return;
  const maeArr = met.mae_dim_mean;
  const mseArr = met.mse_dim_mean;
  if (!Array.isArray(maeArr) && !Array.isArray(mseArr)) return;
  if (Array.isArray(maeArr)) latestMaeDimMean = maeArr;
  if (Array.isArray(mseArr)) latestMseDimMean = mseArr;
  refreshPtDimDiffMeanTable();
  for (const d of state.dims) {
    const t = document.getElementById(`dimTitle-${d}`);
    if (!t) continue;
    const ma =
      Array.isArray(latestMaeDimMean) && typeof latestMaeDimMean[d] === "number"
        ? latestMaeDimMean[d]
        : null;
    const ms =
      Array.isArray(latestMseDimMean) && typeof latestMseDimMean[d] === "number"
        ? latestMseDimMean[d]
        : null;
    t.textContent = `Action dim ${d} · mae_mean ${ma === null ? "-" : ma.toFixed(6)} · mse_mean ${
      ms === null ? "-" : ms.toFixed(6)
    }`;
  }
}
...
function updateTop(event) {
  ...
  if (event.metrics) {
    ...
    if (dualPredMode()) {
      ...
      const maeCum = el("maeCum");
      const mseCum = el("mseCum");
      if (maeCum && typeof m.mae_cum === "number") maeCum.textContent = m.mae_cum.toFixed(6);
      if (mseCum && typeof m.mse_cum === "number") mseCum.textContent = m.mse_cum.toFixed(6);
    }
  }
}
...
function clearClientDisplay() {
  ...
  const maeCum = el("maeCum");
  const mseCum = el("mseCum");
  if (maeCum) maeCum.textContent = "-";
  if (mseCum) mseCum.textContent = "-";
  latestMaeDimMean = null;
  latestMseDimMean = null;
  const ptDimBody = el("ptDimDiffMeanBody");
  if (ptDimBody) ptDimBody.innerHTML = "";
...
function connectInternal() {
  ...
  socket.onmessage = (evt) => {
    ...
    if (msg.type === "step") {
      ...
      updatePerDimDiffMeanFromStep(msg);
    }
  };
}
