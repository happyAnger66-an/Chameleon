...
def test_resolve_compare_mode_from_tvm_runner() -> None:
    task = TaskConfig()
    task.evaluate.policy_runner = "pt_tvm_compare"
    task.evaluate.compare_mode = False
    assert resolve_compare_mode(task) is True
...
def test_build_eval_run_meta_pt_trt_compare() -> None:
    ...
    assert meta["pred2_name"] == "TensorRT"
    assert meta["pair_name"] == "PT−TRT"


def test_build_eval_run_meta_pt_tvm_compare() -> None:
    task = TaskConfig()
    task.evaluate.policy_runner = "pt_tvm_compare"
    meta = build_eval_run_meta(
        task,
    ...
    assert meta["compare_mode"] is True
    assert meta["backend"] == "pt_tvm_compare"
    assert meta["pred2_name"] == "TVM"
    assert meta["pair_name"] == "PT−TVM"
